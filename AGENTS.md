...
2. 사용자가 새로운 독립 작업을 요청할 때마다 기존 사용자 변경을 보존한 상태로 새 `feat/<task-slug>` 브랜치를 만든다. 이전 작업 브랜치에 새 작업을 이어서 커밋하지 않는다.
3. 기본적으로 최신 `main`에서 분기한다. 아직 병합되지 않은 이전 작업에 의존하면 해당 feat 브랜치에서 분기하고 이전 브랜치를 base로 하는 stacked PR을 만든다.
4. 같은 작업의 수정 요청이나 PR 피드백은 새 브랜치를 만들지 않고 해당 작업 브랜치에서 계속 처리한다.
5. 브랜치 하나에는 사용자 작업 하나만 담고 요청 범위 밖의 변경은 스테이징, 되돌리기 또는 리포맷하지 않는다.
6. 아래 변경 이력 규칙에 따라 `CHANGE.md`를 갱신한다.
7. `docs/agent-context/verification.md`의 검증 루프를 통과한다.
8. `git diff`와 `git status`로 최종 변경 범위를 검토한다.
9. 이번 작업에서 직접 변경한 파일만 스테이징하고 Conventional Commits 형식으로 커밋한다. 기능은 `feat:`, 수정은 `fix:`, 문서만 변경하면 `docs:`를 사용한다.
10. 브랜치를 `origin`에 push하고 `main` 대상 PR을 생성한다. stacked PR이면 직접 의존하는 feat 브랜치를 대상으로 한다. PR 본문에는 변경 요약, `CHANGE.md` 항목과 실행한 검증을 기록한다.
11. 완료 보고에 브랜치명, 커밋, PR 링크, PR base와 검증 결과를 포함한다.
